...
class Feeder(torch.utils.data.Dataset):
    def __init__(self, data_path, p_interval=1, split='train', random_choose=False, random_shift=False,
                 random_move=False, random_rot=False, window_size=52, normalization=False, debug=False, use_mmap=False,
    ...
    def load_data(self, mmap=False):
        ...
        num_samples = self.data.shape[0]  # 获取样本数量
        self.sample_name = [f'sample_{i}' for i in range(num_samples)]
        ...
        # self.num_per_cls_dict = [0,]*(np.array(self.lable).max()+1)
        # for i in self.label:
    ...
    def __getitem__(self, index):
        ...
        data_numpy = tools.valid_crop_resize(data_numpy, valid_frame_num, [1], self.window_size)
